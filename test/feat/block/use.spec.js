...
delete __data.define
delete __data.fail
Object.keys(__data).forEach(v => doc.property(v, __data[v]))
// console.log(Object.keys(doc.__data))
doc.registerMethod()

// when the first test,use next line to help you
// fs.writeFileSync(path.join(__dirname, './data-use-expect.txt'), doc.toStr())

const expectData = fs
  .readFileSync(path.join(__dirname, './data-use-expect.txt'))
  .toString()
